...
describe("transcrição manual", () => {
  it("oferece catálogo e quantidades quando a leitura não está disponível", () => {
    render(<ManualProcedureTranscriptionView conferenceId="conference-1" exams={[{ id: "42", name: "HEMOGRAMA", mnemonic: "HEMO" }]} saveAction={vi.fn()} completeAction={vi.fn()} />);
    ...
    expect(screen.getByRole("button", { name: "Concluir transcrição manual" })).toBeVisible();
  });
});
